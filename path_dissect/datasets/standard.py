"""
path_dissect.datasets.standard — standard (non-TCGA) datasets and target model loading.

Contents
--------
DATASET_ROOTS             — mapping of dataset name -> root directory
get_target_model()        — load a torchvision target model + its preprocessing
get_resnet_imagenet_preprocess() — standard ImageNet preprocessing for ResNets
get_data()                — return a Dataset by name
get_places_id_to_broden_label() — utility for Places365 / Broden label mapping
get_cifar_superclass()    — CIFAR-100 superclass split utilities
"""
...
from .tcga import TCGA_TILE_DIR
...
def get_target_model(target_name, device):
    ...
    target_name: supported options —
        resnet18_places, resnet18, resnet34, resnet50, resnet101, resnet152,
        vit_b_16, vit_b_32 (torchvision ViT variants)

    To dissect a different model, add a branch here.
    """
...
def get_data(dataset_name, preprocess=None):
    """
    Return a Dataset for the given dataset_name.

    Supported names: cifar100_train, cifar100_val, imagenet_broden,
    and any key in DATASET_ROOTS (imagenet_val, broden, tcga).
    """
    if dataset_name == "cifar100_train":
        ...
        data = datasets.CIFAR100(
            root=os.path.expanduser("~/.cache"),
            download=True, train=False, transform=preprocess,
        )
    elif dataset_name in DATASET_ROOTS:
